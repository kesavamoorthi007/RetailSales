...
namespace RetailSales.Services.Purchase
{
    public class GRNService : IGRNService
    {
        ...
        public DataTable GetGRNItem(string id)
        {
            ...
            SvSql = "SELECT PRO_DETAIL.SHOP_BIN,PRO_DETAIL.GODOWN_BIN,GRN_DETAIL.VARIANT,GRN_DETAIL.ITEM,GRN_DETAIL.PRODUCT,GRN_BASIC_ID,GRN_DETAIL_ID,PRODUCT.PRODUCT_NAME,PRO_NAME.PROD_NAME,PRO_DETAIL.PRODUCT_VARIANT,HSN,TARIFF,GRN_DETAIL.UOM,QTY,RECIVED_QTY,ACCEPTED_QTY,REJECTED_QTY,EXCEED_QTY,SHORT_QTY,DEST_UOM,CF,CF_QTY,GRN_DETAIL.RATE,AMOUNT,DISC_PER,DIS_AMOUNT,CGSTP,SGSTP,IGSTP,CGST,SGST,IGST,TOTAL_AMOUNT FROM GRN_DETAIL LEFT OUTER JOIN PRODUCT ON PRODUCT.ID=GRN_DETAIL.ITEM LEFT OUTER JOIN PRO_NAME ON PRO_NAME.PRO_NAME_BASICID=GRN_DETAIL.PRODUCT LEFT OUTER JOIN PRO_DETAIL ON PRO_DETAIL.ID=GRN_DETAIL.VARIANT WHERE GRN_DETAIL.GRN_BASIC_ID='" + id + "' AND GRN_DETAIL.RECIVED_QTY >0";
            DataTable dtt = new DataTable();
            SqlDataAdapter adapter = new SqlDataAdapter(SvSql, _connectionString);
            SqlCommandBuilder builder = new SqlCommandBuilder(adapter);
            adapter.Fill(dtt);
