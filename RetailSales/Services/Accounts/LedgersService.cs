...
namespace RetailSales.Services.Accounts
{
    public class LedgersService : ILedgersServices
    {
        ...
        public DataTable GetAllLedgersGRID(string strStatus)
        {
            ...
            {
                SvSql = "SELECT ACC_LEDGER.ID, ACC_GROUP.ACC_GRP_NAME,LEDGER_NAME,ACC_LEDGER.IS_ACTIVE FROM ACC_LEDGER LEFT OUTER JOIN ACC_GROUP ON CAST(ACC_GROUP.ACC_GRP_CODE AS NVARCHAR) = ACC_LEDGER.ACC_GRP_CODE WHERE ACC_LEDGER.IS_ACTIVE = 'N' ORDER BY ACC_LEDGER.ID DESC";
            }
