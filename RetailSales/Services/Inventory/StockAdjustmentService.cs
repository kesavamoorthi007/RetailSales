...
namespace RetailSales.Services.Inventory
{
    ...
    TYPE,
    DOCID,
    CONVERT(VARCHAR, STKADJBASIC.DOCDATE, 106) AS DOCDATE,
    STKADJBASIC.IS_ACTIVE,
    (
        SELECT STRING_AGG(P.PRODUCT_NAME + ' - ' + PN.PROD_NAME, ', ')
        FROM STKADJDETAIL SD
        LEFT JOIN PRO_NAME PN ON SD.PRODUCT = PN.PRO_NAME_BASICID
        LEFT JOIN PRODUCT P ON P.ID = SD.PRODUCT_NAME
        WHERE SD.STKADJBASICID = STKADJBASIC.STKADJBASICID
    ) AS PRODUCT
FROM STKADJBASIC
LEFT OUTER JOIN LOCATION ON LOCATION.ID = STKADJBASIC.LOCATION
WHERE STKADJBASIC.IS_ACTIVE = '"+ strStatus + "' ORDER BY STKADJBASIC.STKADJBASICID DESC";
