...
namespace RetailSales.Services.Sales
{
    public class SalesInvoiceService : ISalesInvoiceService
    {
        ...
        {
            string SvSql = string.Empty;
            SvSql = "SELECT PRO_DETAIL.ID,PRODUCT_VARIANT FROM PRO_DETAIL WHERE PRO_DETAIL.PRODUCT_ID='" + id + "' AND PRO_DETAIL.IS_ACTIVE = 'Y' AND PRODUCT_VARIANT IS NOT NULL";
            DataTable dtt = new DataTable();
            SqlDataAdapter adapter = new SqlDataAdapter(SvSql, _connectionString);
            SqlCommandBuilder builder = new SqlCommandBuilder(adapter);
            adapter.Fill(dtt);
        ...
        {
            string SvSql = string.Empty;
            SvSql = "  SELECT SAL_INV_DETAILID,SAL_INV_BASICID,PRODUCT.PRODUCT_NAME,PRO_NAME.PROD_NAME,PRO_DETAIL.PRODUCT_VARIANT, CONCAT(PRODUCT.PRODUCT_NAME,' ',PRO_NAME.PROD_NAME,' ',PRO_DETAIL.PRODUCT_VARIANT) AS FULL_PRODUCT_NAME,SAL_INV_DEATILS.HSN_CODE,SAL_INV_DEATILS.UOM,BIN_NO,QTY,DEST_UOM,CF,CF_QTY,SAL_INV_DEATILS.RATE,AMOUNT,DISC_PER,DISCOUNT,TOTAL FROM SAL_INV_DEATILS LEFT OUTER JOIN PRODUCT ON PRODUCT.ID=SAL_INV_DEATILS.ITEM LEFT OUTER JOIN PRO_NAME ON PRO_NAME.PRO_NAME_BASICID=SAL_INV_DEATILS.PRODUCT LEFT OUTER JOIN PRO_DETAIL ON PRO_DETAIL.ID=SAL_INV_DEATILS.VARIENT WHERE SAL_INV_DEATILS.SAL_INV_BASICID =" + id + "";
            DataTable dtt = new DataTable();
            SqlDataAdapter adapter = new SqlDataAdapter(SvSql, _connectionString);
            SqlCommandBuilder builder = new SqlCommandBuilder(adapter);
            adapter.Fill(dtt);
