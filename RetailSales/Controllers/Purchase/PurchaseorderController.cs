...
namespace RetailSales.Controllers.Purchase
{
    public class PurchaseorderController : Controller
    {
        ...
        {
            Purchaseorder ic = new Purchaseorder();
            ic.Categorylst = BindProdCategory();
        ...
        public ActionResult AddProductName(Purchaseorder cy, string id)
        {
            try
            {
                cy.ID = id;
                string Strout = PurchaseorderService.ProductNameCRUD(cy);
                if (string.IsNullOrEmpty(Strout))
                {
                    if (cy.ID == null)
                    {
                        TempData["notice"] = "ProductName Inserted Successfully...!";
                    }
                    else
                    {
                        TempData["notice"] = "ProductName Inserted Successfully...!";
                ...
                {
                    ViewBag.PageTitle = "Edit ProductName";
                    TempData["notice"] = Strout;
                }
        ...
        private List<SelectListItem> BindProdCategory()
        {
            try
            {
                DataTable dtDesg = PurchaseorderService.GetProdCategory();
        ...
        public List<SelectListItem> BindHsn()
        {
            try
            {
                DataTable dtDesg = PurchaseorderService.GetHsn();
                List<SelectListItem> lstdesg = new List<SelectListItem>();
                for (int i = 0; i < dtDesg.Rows.Count; i++)
                {
        ...
        //public JsonResult SaveProductName(string Category, string Product, string Description, string Varient, string UOM, string Hsn, String MinQty, string Rate, string ProdDesc)
        //{

        //    string id = PurchaseorderService.ProductNameCRUD(Category, Product, Description, Varient, UOM, Hsn, MinQty, Rate, ProdDesc);
        ...
        public List<SelectListItem> BindState()
        {
            try
            {
                DataTable dtDesg = PurchaseorderService.GetState();
                List<SelectListItem> lstdesg = new List<SelectListItem>();
                for (int i = 0; i < dtDesg.Rows.Count; i++)
                {
        ...
        public List<SelectListItem> BindCity(string cityid)
        {
            try
            {
                DataTable dtDesg = PurchaseorderService.GetCity(cityid);
                List<SelectListItem> lstdesg = new List<SelectListItem>();
                for (int i = 0; i < dtDesg.Rows.Count; i++)
                {
        ...
        public List<SelectListItem> BindCategory()
        {
            try
            {
                DataTable dtDesg = PurchaseorderService.GetCategory();
                List<SelectListItem> lstdesg = new List<SelectListItem>();
                for (int i = 0; i < dtDesg.Rows.Count; i++)
                {
        ...
        public JsonResult SaveSupplier(string Category, string SupplierName, string SupplierAdd, string Days, string GST, string State, String City, string Mobile, string Landline, string Email)
        {
            ...
            string id = PurchaseorderService.SupplierCRUD(Category, SupplierName, SupplierAdd, Days, GST, State, City, Mobile, Landline, Email);
            var result = new { id = id };
            return Json(result);
        }
...
<br/102-A
        ...
        public IActionResult MoveGRN(string id)
        {
            ...
            if (dtt.Rows.Count > 0)
            {
                ...
                {
                    tda = new PurchaseorderItem();
                    tda.DUOMlst = BindDUOM(dtt.Rows[i]["ID"].ToString());
                    tda.Item = dtt.Rows[i]["PRODUCT_NAME"].ToString();
